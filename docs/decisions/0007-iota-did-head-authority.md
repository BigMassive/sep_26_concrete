...
## Implementation stages (code in a later session)

| Stage | Intent |
|-------|--------|
| **A** | Identity configured ⇒ create/advance fail if chain does not accept the new head |
| **B** | `GET` hydrates head from Identity resolve, not from OTP `head_cid` as authority |
| **C** | New objects are iota-only (`did` in the API **is** `did:iota:…`) |
| **D** | Registry shrinks to an index (iota DID, ControllerCap id, label); rebuildable; no authoritative head field |
...
- Beat 2 / 6 / Godot plaque contracts must switch primary DID to `did:iota:…` when C lands.
