...
4. **Required toolchain (no cost):** Git, Docker Engine + Compose, Godot Editor, **Elixir + OTP**, and related OSS build tools. Optional: VSCodium, Neovim, etc. **Cursor is not required.**
5. **Clone-and-go bar:** README documents a short path: clone → install Godot + Elixir/OTP → `docker compose up` → run the BEAM app → open the Godot project. No proprietary accounts required for that path.
...
- Open decision “UI technology” is resolved toward Godot; runtime is **Elixir / BEAM** ([ADR 0003](0003-runtime-beam.md)).
